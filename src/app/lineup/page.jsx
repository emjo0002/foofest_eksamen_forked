    ...
    <div className="wrapper grid overflow-x-auto overscroll-contain scrollbar-hide scroll-snap-x mandatory py-8">
      <div className="items flex gap-4 pl-[calc(50vw-12rem)] pr-[calc(50vw-12rem)]">
        {bands.map((band) => (
          <div
            key={band.id}
            className="bg-blue-800 grid place-content-center rounded-lg shadow-lg hover:shadow-2xl transition duration-300 scroll-snap-center w-96"
          >
            <Image
              src={`/logos/${band.logo}`}
              width={275}
              height={250}
              alt={band.slug}
              className="h-64 object-fill rounded-md"
            />
